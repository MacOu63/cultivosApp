        ...
        <div class="col-md-12">
            <div class="card card-default">
                <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                    <span class="card-title">{{ __('Crear') }} nueva Anunciante</span>
                    <a class="btn btn-primary btn-sm" href="{{ route('anunciantes.index') }}"> {{ __('Regresar') }}</a>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('anunciantes.store') }}" role="form" enctype="multipart/form-data">
                        @csrf
