        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header d-flex justify-content-between align-items-center" style="background-color: #f8f9fa; padding: 10px; border-bottom: 1px solid #ddd;">
                    <div>
                        <button class="btn btn-primary btn-sm" title="Crear Nuevo" data-placement="left" onclick="window.location.href='{{ route('cultivos.create') }}'">
                    ...
                    <div class="d-flex align-items-center" style="flex-grow: 1; justify-content: flex-end;">
                        <div class="input-group mr-3" style="max-width: 400px;">
                            ...
                            <form action="{{ route('cultivos.index') }}" method="GET">
                                <div class="row">
                                    <div class="col-8">
                                        <input type="text" name="search" class="form-control" placeholder="Ingresar nombre de cultivo:" value="{{ request('search') }}" id="search-input" style="margin-right:10px";>
                                    </div>
                                    <div class="col-4">
                                        <button class="btn btn-outline-secondary rounded-right w-100" type="submit" style="background-color: #007bff; border-color: #007bff; color: white; margin-right:-100px !important; width: 155px !important">
                                            Buscar por cultivo
                                            <!-- Icono SVG -->
                    ...
                    <form action="{{ route('cultivos.index') }}" method="GET" id="results-form">
                ...
                <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th style="width: 13%;">Cultivos</th>
                                    <th style="width: 19%;">Fecha de Creación</th>
                                    <th style="width: 19%;">Última Actualización</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td class="d-flex justify-content-start actions-btns">
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        <a href="{{ route('cultivos.show', $cultivo->id) }}" class="btn btn-sm" style="background-color: #4CAF50; color: white;">
                                        ...
                                        <a href="{{ route('cultivos.edit', $cultivo->id) }}" class="btn btn-sm btn-warning">
                                        ...
                                        <form action="{{ route('cultivos.destroy', $cultivo->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <td>
                                        @if($cultivo->created_at)
                                            {{ $cultivo->created_at->format('d F Y h:i a') }}
                                        @else
                                            N/D
                                        @endif
                                    </td>
                                    <td>
                                        @if($cultivo->updated_at)
                                            {{ $cultivo->updated_at->format('d F Y h:i a') }}
                                        @else
                                            N/D
                                        @endif
                                    </td>
                    ...
                    <br>
                    {!! $cultivos->withQueryString()->links() !!}
                </div>
            </div>
        </div>
    </div>
...
<style>
    ...
    @media (min-width: 577px) and (max-width: 768px) {
        ...
        .alert-warning {
            width: 200px; /* Permitir que el ancho sea completo en pantallas pequeñas */
